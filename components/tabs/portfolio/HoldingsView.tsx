...
export const HoldingsView: React.FC<HoldingsViewProps> = ({
  ...
  searchTerm, filterType, groupBy, viewMode
}) => {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (e.key === 'ArrowDown') {
            e.preventDefault();
            setSelectedRowIndex(prev => Math.min(prev + 1, filteredInvestments.length - 1));
        } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            setSelectedRowIndex(prev => Math.max(prev - 1, 0));
        } else if (e.key === 'Enter') {
            e.preventDefault();
            if (selectedRowIndex >= 0 && filteredInvestments[selectedRowIndex]) {
                setSimulatorAsset(filteredInvestments[selectedRowIndex]);
            }
        }
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [viewMode, filteredInvestments, selectedRowIndex, editingCell]);
  ...
  const renderCardItem = (inv: Investment) => {
      ...
      return (
          <div key={inv.id} onClick={() => setSimulatorAsset(inv)} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-indigo-200 dark:hover:border-indigo-800 transition-all duration-300 group relative overflow-hidden hover:shadow-lg dark:hover:shadow-[0_0_20px_rgba(99,102,241,0.1)] cursor-pointer">
    ...
    <div className="space-y-4">
      {viewMode === 'TERMINAL' && (
          <div className="flex items-center gap-4 text-xs text-slate-500 px-2 mb-2 animate-in fade-in">
              ...
              <div className="flex items-center gap-1"><span className="p-1 bg-slate-200 dark:bg-slate-800 rounded">Click</span> <span>Edit</span></div>
          </div>
      )}
      ...
      {investments.length === 0 ? (
          ...
          viewMode === 'TERMINAL' ? (
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
                  <table className="w-full text-left border-collapse">
                      ...
                      <tbody>{filteredInvestments.map((inv, index) => renderTerminalRow(inv, index))}</tbody>
                  </table>
              </div>
          ) : (
          ...
          groupedInvestments && Object.entries(groupedInvestments).map(([groupKey, groupData]: [string, GroupedData]) => (
              <div key={groupKey} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                  <div className="sticky top-[80px] z-20 p-4 bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700 flex items-center justify-between cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800" onClick={() => toggleGroup(groupKey)}>
                      <div className="flex items-center gap-3">
                          <div className={`p-1.5 rounded-lg ${expandedGroups[groupKey] ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400' : 'text-slate-400'}`}>{expandedGroups[groupKey] ? <ChevronDown size={18} /> : <ChevronRight size={18} />}</div>
                          <div><h3 className="font-bold text-slate-800 dark:text-white text-base">{groupKey}</h3><p className="text-xs text-slate-500">{groupData.items.length} assets • Total: {formatCurrency(groupData.totalCurr)}</p></div>
                      </div>
                      <div className="text-right"><p className={`text-sm font-mono font-bold ${(groupData.totalCurr - groupData.totalInv) >= 0 ? 'text-emerald-600 dark:text-emerald-500' : 'text-rose-600 dark:text-rose-500'}`}>{(groupData.totalCurr - groupData.totalInv) >= 0 ? '+' : ''}{calculatePercentage(groupData.totalCurr - groupData.totalInv, groupData.totalInv)}%</p></div>
                  </div>
                  {expandedGroups[groupKey] && (
                      <div className="p-4 animate-in slide-in-from-top-2 duration-200">
                          {viewMode === 'TERMINAL' ? (
                              <table className="w-full text-left border-collapse"><tbody>{groupData.items.map((inv, idx) => renderTerminalRow(inv, idx))}</tbody></table>
                          ) : (
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{groupData.items.map(inv => renderCardItem(inv))}</div>
                          )}
                      </div>
                  )}
              </div>
