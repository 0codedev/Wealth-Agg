...
import { 
  ...
  BarChart3, ArrowRight, MousePointerClick, Thermometer,
  Activity, TrendingUp, LayoutGrid, List, Clover,
  ArrowRightLeft, Timer, Briefcase, CandlestickChart, Scale
} from 'lucide-react';
...
const IPOWarRoom: React.FC<IPOWarRoomProps> = ({ investments = [], onRefresh }) => {
  ...
  // --- Feature 3: Probability Engine ---
  const [subscriptionX, setSubscriptionX] = useState<number | ''>('');
  ...
  // --- Feature 6: The Flipper's Matrix ---
  ...
  const isSkipSignal = (subProb !== '' && currentGmp !== '') && (Number(subProb) < 1 && Number(currentGmp) < 20);
  ...
  const trustScore = useMemo(() => {
      const q = Number(qibSub) || 0;
      let score = 0;
      if (q >= 50) score = 100;
      else if (q >= 10) score = 70 + ((q - 10) / 40) * 30;
      else score = q * 7;
  ...
  const flipperStats = useMemo(() => {
      // 1. IPO Return
      // Absolute = Expected GMP
      // Annualized = (GMP / Capital) * (365 / LockIn)
      const ipoAbsReturn = expectedGmpVal;
      const ipoRoi = (ipoAbsReturn / flipperAmount);
      const ipoAnnualized = ipoRoi * (365 / lockInDays) * 100;

      // 2. Swing Trade
      // Absolute = Capital * (Swing% / 100)
      // Annualized = (Swing% / 100) * (365 / LockIn) * 100
      const swingAbsReturn = flipperAmount * (swingReturnPct / 100);
      const swingAnnualized = (swingReturnPct / 100) * (365 / lockInDays) * 100;

      const isIPOBetter = ipoAbsReturn > swingAbsReturn;

      return { ipoAbsReturn, ipoAnnualized, swingAbsReturn, swingAnnualized, isIPOBetter };
  }, [flipperAmount, lockInDays, expectedGmpVal, swingReturnPct]);
    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-20 md:pb-0">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p className="text-slate-400 mt-1">
                    Forensic DRHP Analyzer & Syndicate Tracker.
                </p>
            </div>
      ...
      {activeSubTab === 'SYNDICATE' ? (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              {/* Quick Fill Presets */}
              <div className="flex flex-wrap gap-2 items-center bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm relative overflow-hidden">
                      <div className="flex flex-col md:flex-row gap-6">
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                  <div className={`p-3 rounded-xl border transition-all ${getHeatColor(qibSub, 'QIB')}`}>
                                      ...
                                      <input 
                                          ...
                                          placeholder="0.0"
                                          className="w-full bg-transparent text-xl font-black outline-none placeholder:opacity-50"
                                      />
                                  </div>
                          ...
                          <div className="flex-1 bg-slate-50 dark:bg-slate-950 rounded-xl p-4 border border-slate-100 dark:border-slate-800 flex items-center gap-6">
                              <div className="relative w-24 h-24 flex items-center justify-center shrink-0">
                                  <svg className="w-full h-full transform -rotate-90">
                                      ...
                                      <circle cx="48" cy="48" r="40" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray={251.2} strokeDashoffset={251.2 - (trustScore / 100 * 251.2)} className={`${trustScore > 75 ? 'text-emerald-500' : trustScore > 40 ? 'text-amber-500' : 'text-rose-500'} transition-all duration-1000`} strokeLinecap="round" />
                                  </svg>
                                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                                      <span className="text-2xl font-black text-slate-800 dark:text-white">{trustScore}</span>
                                  </div>
                              </div>
                              <div>
                                  <h4 className="font-bold text-slate-800 dark:text-white mb-1 flex items-center gap-2">
                                      Smart Money Trust
                                      {trustScore > 80 && <CheckCircle2 size={16} className="text-emerald-500" />}
                                  </h4>
                      ...
                      <div className="flex flex-col md:flex-row gap-6 relative z-10">
                          <div className="flex-1">
                              ...
                              <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800">
                                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Subscription Level (Times)</label>
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-4">
                              ...
                              <div className={`p-4 rounded-xl border-2 flex flex-col justify-between ${flipperStats.isIPOBetter ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500' : 'bg-slate-50 dark:bg-slate-950 border-transparent'}`}>
                                  ...
                                  <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                                      <p className="text-xs text-slate-500">Absolute: <span className="font-mono font-bold text-emerald-500">+{formatCurrency(flipperStats.ipoAbsReturn)}</span></p>
                                  </div>
                              </div>
                              ...
                              <div className={`p-4 rounded-xl border-2 flex flex-col justify-between ${!flipperStats.isIPOBetter ? 'bg-indigo-50 dark:bg-indigo-900/10 border-indigo-500' : 'bg-slate-50 dark:bg-slate-950 border-transparent'}`}>
                                  ...
                                  <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                                      <p className="text-xs text-slate-500">Absolute: <span className="font-mono font-bold text-indigo-500">+{formatCurrency(flipperStats.swingAbsReturn)}</span></p>
                                  </div>
                              </div>
                          </div>
                      </div>
