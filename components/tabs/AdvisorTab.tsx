...
import { 
  ...
  Sparkles, Star, ChevronDown, Check, ChevronUp, Maximize2
} from 'lucide-react';
...
const MODELS = [
    ...
    { id: 'gemini-3-pro-preview', label: 'Pro', icon: Star, description: 'Smartest' }, 
];
...
const VoiceOrb = ({ isActive }: { isActive: boolean }) => {
    return (
        <div className="relative w-32 h-32 flex items-center justify-center">
            <div className={`absolute w-16 h-16 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-500 shadow-[0_0_30px_rgba(99,102,241,0.6)] z-20 transition-all duration-500 ${isActive ? 'scale-110 shadow-[0_0_50px_rgba(99,102,241,0.8)]' : 'scale-100'}`}></div>
            <div className={`absolute w-24 h-24 rounded-full border-2 border-indigo-400/30 border-t-indigo-400 z-10 transition-all duration-1000 ${isActive ? 'animate-[spin_3s_linear_infinite]' : ''}`}></div>
            <div className={`absolute w-32 h-32 rounded-full border border-purple-400/20 border-b-purple-400 z-0 transition-all duration-1000 ${isActive ? 'animate-[spin_5s_linear_infinite_reverse]' : ''}`}></div>
            {isActive && (
                <>
...
const ChatBubble: React.FC<ChatBubbleProps> = ({ role, text, image }) => {
    ...
    return (
        <div className={`flex w-full mb-6 ${isUser ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className={`flex ${isUser ? 'max-w-[80%]' : 'max-w-[98%] w-full'} gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
...
const AdvisorTab: React.FC<AdvisorTabProps> = ({ investments, totalNetWorth, onNavigate }) => {
  ...
  const handleToggleLiveSession = async () => {
      if (isLiveSessionActive) {
          ...
          try {
              const systemInstructions = `You are a financial advisor. Net Worth: ${totalNetWorth}. Portfolio: ${JSON.stringify(investments)}. Answer questions concisely.`;
  ...
  const handleSendMessage = async () => {
      ...
      try {
          ...
          setChatMessages(prev => [...prev, { role: 'model', text: "Error: Unable to process request with selected model.", image: null }]);
      } finally {
          setIsSending(false);
      }
  };
  ...
  return (
      <div className="h-full flex flex-col relative bg-slate-50 dark:bg-slate-950">
          ...
          <div className="sticky top-0 z-30 flex items-center justify-between p-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm transition-all">
          ...
          {activeTab === 'CHAT' ? (
                  ...
                  <div className="flex-1 overflow-y-auto p-4 md:p-6 scroll-smooth pb-24">
                      {chatMessages.length === 0 && !advisorData ? (
                          <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-60">
                              <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-500 rounded-2xl flex items-center justify-center mb-6 shadow-sm">
                                  <Bot size={40} />
                              </div>
                              ...
                              <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                                  {["Analyze my risk exposure", "Upload Chart Screenshot", "Review portfolio diversity", "How to save tax?"].map((q, i) => (
                                      <button 
                                        key={i} 
                          ...
                          <div className="w-full max-w-5xl mx-auto">
                              ...
                              {isSending && (
                                  <div className="flex w-full mb-6">
                                      <div className="flex items-center gap-2 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl rounded-tl-sm">
              ...
              <div className="h-full flex items-center justify-center p-8 animate-in fade-in slide-in-from-right-4">
                  {/* Voice Interface */}
