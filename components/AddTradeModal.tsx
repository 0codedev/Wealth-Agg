...
import { X, ChevronRight, ChevronLeft, Save, Brain, Calculator, Image as ImageIcon, Upload, Target, Zap, AlertTriangle, ShieldAlert, Crosshair, BarChart2, Loader2, Frown, Smile, Meh, Divide, Award, Clock, CheckSquare, ListChecks } from 'lucide-react';
import { db, Trade, TradeSetup, SetupGrade, Strategy } from '../database';
...
const AddTradeModal: React.FC<AddTradeModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [checkedRules, setCheckedRules] = useState<boolean[]>([]); // Array of booleans mapping to selectedStrategy.rules
  ...
  useEffect(() => {
      if (formData.setup) {
          ...
          if (strat) {
              ...
              // Only reset checklist if switching to a new strategy (simple check: length diff or just reset)
              // For simplicity, we reset if rules length mismatches, but a better way is to track prev ID
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4">
      <div 
        className={`w-full max-w-2xl bg-slate-900 border rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-all duration-300 
      ...
      >
            ...
            {step === 1 && (
              <motion.div key="step1" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-6">
                ...
                <div className="space-y-5">
                    ...
                    {selectedStrategy && (
                        <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 animate-in fade-in slide-in-from-top-2">
            ...
            {step === 3 && (
              <motion.div key="step3" variants={variants} initial="enter" animate="center" exit="exit" className="space-y-6">
                <div className="text-center mb-6">
                    ...
                    <p className="text-slate-400 text-sm">{riskPerTrade}% Max Risk Rule Enforced.</p>
                </div>
        ...
        {/* Footer */}
        <div className="p-6 border-t border-slate-800 bg-slate-950 flex justify-between z-20">
